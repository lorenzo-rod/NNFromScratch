...
class LinearLayer:
    ...
    def reset_gradients(self):
        self.grad_weights = np.zeros((self.output_dim, self.input_dim))
        self.grad_biases = np.zeros(self.output_dim)
...
class FeedForwardNetwork:
    ...
    def backward(self, y_true, y_pred):
        ...
        self.layers[-2].linear.grad_biases += np.where(self.layers[-2].linear.z > 0, self.layers[-1].weights, 0).squeeze() * upstream_gradient
        ...
        for layer in reversed(self.layers[:-2]):
            weights_diagonal = np.diagonal(self.layers[self.layers.index(layer) + 1].linear.weights)
            layer.linear.grad_biases += (np.where(layer.linear.z > 0, weights_diagonal, 0) * upstream_gradient).squeeze()
